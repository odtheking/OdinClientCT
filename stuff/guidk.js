...
const drawTab = (tab) => {
    ...
    Object.entries(tab).forEach((key, i, value) => {
        ...
        if (key[1].toggle) {
            centeredString(font2, key[1].name, tab.x, tab.y + (buttonHeight * (i - 2)), 1, 1, 1, 1)
        } else {
...
const checkTab = (mx, my, b, tab) => {
    ...
    const key = Object.keys(tab)[toChange + 3]
    modMessage(Object.keys(key))
    modMessage('Changing ' + Object.keys(tab)[toChange+3].name + ' to ' + !Object.keys(tab)[toChange + 3].toggle)
    if (b == 0 && toChange >= 0 && toChange <= tab.length - 3) {
        
        Object.keys(tab)[toChange+3].toggle = !Object.keys(tab)[toChange + 3].toggle
